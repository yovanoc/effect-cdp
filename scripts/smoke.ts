...
import { Cause, Duration, Effect, Exit, Fiber, Metric, Ref, Schema, Stream } from "effect"
...
class NoTargetsError extends Schema.TaggedErrorClass<NoTargetsError>("NoTargetsError")(
  "NoTargetsError",
  { message: Schema.String }
) {}

class MissingWebSocketUrlError extends Schema.TaggedErrorClass<MissingWebSocketUrlError>("MissingWebSocketUrlError")(
  "MissingWebSocketUrlError",
  { message: Schema.String }
) {}

class AssertionError extends Schema.TaggedErrorClass<AssertionError>("AssertionError")(
  "AssertionError",
  { message: Schema.String }
) {}

class ChromiumNotFoundError extends Schema.TaggedErrorClass<ChromiumNotFoundError>("ChromiumNotFoundError")(
  "ChromiumNotFoundError",
  { message: Schema.String }
) {}

class ChromiumLaunchTimeoutError extends Schema.TaggedErrorClass<ChromiumLaunchTimeoutError>("ChromiumLaunchTimeoutError")(
...
const runDetach = Effect.fnUntraced(function*(targetId: string) {
  ...
  const childTarget = yield* cdp.session(parentSessionId).send(GeneratedTarget.createTarget, {
    url: "about:blank"
  })
  ...
  if (!Exit.isFailure(exit)) {
    return yield* new AssertionError({
      message: "expected child-session send to fail after detach, but it succeeded"
    })
  }
  ...
  if (!(error instanceof CdpDisconnected) || error.reason !== "TargetDetached") {
    return yield* new AssertionError({
      message: `expected CdpDisconnected{reason:'TargetDetached'}, got: ${Cause.pretty(exit.cause)}`
    })
  }
  yield* Effect.log("OK child session failed with CdpDisconnected{reason:'TargetDetached'}")
...
const runTimeout = Effect.fnUntraced(function*(targetId: string) {
  ...
  const outcome = yield* cdp.session(sessionId).send(
...
const runMultiSession = Effect.fnUntraced(function*(targetId: string) {
  ...
  const created = yield* cdp.session(sessionA).send(GeneratedTarget.createTarget, {
    url: "about:blank"
  })
  ...
  yield* Effect.log(`OK distinct sessions sessionA=${sessionA} sessionB=${sessionB}`)
  ...
  const containsMarker = (event: { readonly params: unknown }): boolean => {
    ...
    const args = (params as { readonly args?: ReadonlyArray<{ readonly value?: unknown }> }).args
    if (!Array.isArray(args)) return false
    for (const arg of args) {
      if (arg !== null && typeof arg === "object" && "value" in arg && (arg as { value?: unknown }).value === marker) {
        return true
      }
    }
    return false
  ...
  yield* Effect.log(`sessionA marker events=${countA} sessionB marker events=${countB}`)
  ...
  yield* Effect.log(`OK multi-session isolation: sessionA=${countA} sessionB=${countB}`)
})
...
const runDiscovery = Effect.fnUntraced(function*(host: string) {
  ...
  yield* Effect.gen(function*() {
    ...
    if (typeof version["Protocol-Version"] !== "string" || version["Protocol-Version"].length === 0) {
      return yield* new AssertionError({
    ...
    yield* Effect.log(`OK version Browser=${version.Browser} Protocol-Version=${version["Protocol-Version"]}`)
    ...
    if (typeof first.webSocketDebuggerUrl !== "string" || first.webSocketDebuggerUrl.length === 0) {
      return yield* new AssertionError({
        message: `list()[0].webSocketDebuggerUrl missing or empty, got ${JSON.stringify(first.webSocketDebuggerUrl)}`
      })
    }
    yield* Effect.log(`OK list returned ${targets.length} target(s), first id=${first.id} type=${first.type}`)
...
const findChromium = (override: string | undefined): Effect.Effect<string, ChromiumNotFoundError> =>
  Effect.sync(() => {
    if (override !== undefined && override.length > 0) return override
    for (const cmd of ["chromium", "google-chrome", "chrome"]) {
      try {
        const found = execSync(`which ${cmd}`, { stdio: ["ignore", "pipe", "ignore"] }).toString().trim()
    ...
    Effect.flatMap((p) =>
      p === null
        ? new ChromiumNotFoundError({ message: "could not locate chromium/google-chrome on PATH; pass --chromium-path=" })
        : Effect.succeed(p))
  )

const waitForChromium = (host: string): Effect.Effect<void, ChromiumLaunchTimeoutError> => {
...
const runDisconnect = Effect.fnUntraced(function*(chromiumPath: string | undefined) {
  ...
  const work = Effect.gen(function*() {
    ...
    const start = Date.now()
    const exit = yield* cdp.session(sessionId).send(GeneratedRuntime.evaluate, {
      expression: "new Promise(() => {})",
      awaitPromise: true,
      returnByValue: true
    }).pipe(Effect.exit)
    ...
    if (!Exit.isFailure(exit)) {
      return yield* new AssertionError({
        message: "expected Runtime.evaluate to fail after SIGKILL, but it succeeded"
      })
    }
    const failureOption = Cause.findErrorOption(exit.cause)
    if (failureOption._tag === "None") {
    ...
    const error = failureOption.value
    if (!(error instanceof CdpDisconnected) || error.reason !== "SocketClosed") {
      return yield* new AssertionError({
    ...
    yield* Effect.log(`OK CdpDisconnected reason=SocketClosed elapsed=${elapsed}ms (sendStart→fail=${failedAt - start}ms)`)
  })
...
const program = Effect.fnUntraced(function*() {
  ...
  switch (test) {
    case "default":
      yield* runDefault(targetId).pipe(Effect.provide(Cdp.layerBun(config)), Effect.scoped)
      return
    case "detach":
      yield* runDetach(targetId).pipe(Effect.provide(Cdp.layerBun(config)), Effect.scoped)
      return
    case "timeout":
      yield* runTimeout(targetId).pipe(Effect.provide(Cdp.layerBun(config)), Effect.scoped)
      return
    case "backpressure":
      yield* runBackpressure(targetId).pipe(Effect.provide(Cdp.layerBun(config)), Effect.scoped)
      return
    case "multi-session":
      yield* runMultiSession(targetId).pipe(Effect.provide(Cdp.layerBun(config)), Effect.scoped)
      return
    default:
