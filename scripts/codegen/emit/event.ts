...
const emitParams = (domain: string, evt: EventDef, cyclicDomains?: ReadonlySet<string>): string => {
  const fields = evt.parameters?.map((property) => {
...
export function emitEvent(domain: string, evt: EventDef, cyclicDomains?: ReadonlySet<string>): string {
