...
import { CdpDecodeError, CdpDisconnected, CdpProtocolError, CdpTimeout } from "../errors.js"
import { CdpRequestId, type SessionId } from "../types.js"

type PendingDrainReason = typeof CdpDisconnected.Type["reason"]
type PendingError = CdpProtocolError | CdpDecodeError | CdpDisconnected | CdpTimeout
...
export class PendingMap extends Context.Service<PendingMap, {
  readonly register: (
    id: CdpRequestId,
    sessionId: Option.Option<SessionId>,
    method: string
  ) => Effect.Effect<Deferred.Deferred<unknown, PendingError>, CdpDisconnected>
  readonly complete: (id: CdpRequestId, result: unknown) => Effect.Effect<void>
  readonly fail: (id: CdpRequestId, error: PendingError) => Effect.Effect<void>
  readonly cancel: (id: CdpRequestId) => Effect.Effect<void>
  readonly drainAll: (reason: PendingDrainReason) => Effect.Effect<void>
  readonly drainSession: (sessionId: SessionId, reason: PendingDrainReason) => Effect.Effect<void>
  readonly nextId: () => Effect.Effect<CdpRequestId>
}>()("effect-cdp/internal/PendingMap") {
  static readonly make: Effect.Effect<PendingMap["Service"]> = Effect.gen(function*() {
    ...
    const register = Effect.fnUntraced(function*(
      ...
      const result = yield* Ref.modify(stateRef, (state): [boolean, PendingState] => {
        if (state._tag === "Closed") {
          return [false, state]
        }
        const entries = HashMap.set(state.entries, id, { deferred, sessionId, method })
    ...
    const takeEntry = (id: CdpRequestId) =>
      Ref.modify(stateRef, (state): [Option.Option<PendingEntry>, PendingState] => {
        if (state._tag === "Closed") {
    ...
    const complete = Effect.fnUntraced(function*(id: CdpRequestId, result: unknown) {
    ...
    const fail = Effect.fnUntraced(function*(id: CdpRequestId, error: PendingError) {
      const entry = yield* takeEntry(id)
      if (Option.isSome(entry)) {
    ...
    const drainAll = Effect.fnUntraced(function*(reason: PendingDrainReason) {
    ...
    const drainSession = Effect.fnUntraced(function*(sessionId: SessionId, reason: PendingDrainReason) {
      const entries = yield* Ref.modify(stateRef, (state): [ReadonlyArray<PendingEntry>, PendingState] => {
        if (state._tag === "Closed") {
          return [[], state]
        }
        const matching = Array.from(HashMap.values(state.entries)).filter((entry) =>
          matchesSession(entry, sessionId)
        )
        const remaining = HashMap.filter(state.entries, (entry) => !matchesSession(entry, sessionId))
        return [matching, { _tag: "Open", entries: remaining, counter: state.counter }]
      })
      const error = disconnected(reason)
      yield* Effect.forEach(
        entries,
  ...
  static readonly layer: Layer.Layer<PendingMap> = Layer.effect(PendingMap, PendingMap.make)
}
