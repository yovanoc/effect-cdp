...
type SendError = CdpDisconnected | CdpTimeout | CdpProtocolError | CdpDecodeError

const timeoutError = (method: string, requestId: CdpRequestId, timeout: Duration.Duration): CdpTimeout =>
  new CdpTimeout({
    method,
    requestId,
...
const shouldIncludeParams = (params: unknown): boolean =>
  params !== undefined && Predicate.isObject(params) && Object.keys(params).length > 0
...
export const send = Effect.fnUntraced(function*(
  ...
  method: string,
  params: unknown,
  opts: { readonly sessionId?: SessionId; readonly timeout?: Duration.Duration }
): Effect.fn.Return<unknown, SendError> {
  const id = yield* pending.nextId()
  const sessionId = shouldRouteFromRoot(method, params) ? undefined : opts.sessionId
  ...
  yield* Effect.scoped(
    ...
    Effect.catch(() => {
      ...
      return pending.fail(id, error).pipe(
