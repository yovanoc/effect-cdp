...
export class HttpDiscovery extends Context.Service<HttpDiscovery, {
  ...
  readonly newTab: (url?: string) => Effect.Effect<CdpTargetInfo, CdpDecodeError>
  ...
  static readonly make = (
    ...
    Effect.gen(function*() {
      ...
      const version = Effect.fn("HttpDiscovery.version")(
        function*() {
          const response = yield* client.get(`${baseUrl}/json/version`).pipe(
            Effect.catchTag("HttpClientError", Effect.die)
          )
          return yield* decodeBody(CdpVersionInfo, response)
        }
      )

      const list = Effect.fn("HttpDiscovery.list")(
        function*() {
          const response = yield* client.get(`${baseUrl}/json/list`).pipe(
            Effect.catchTag("HttpClientError", Effect.die)
          )
      ...
      const newTab = Effect.fn("HttpDiscovery.newTab")(
        function*(url?: string) {
          const path = url === undefined
            ? `${baseUrl}/json/new`
            : `${baseUrl}/json/new?${encodeURIComponent(url)}`
          const response = yield* client.put(path).pipe(
            Effect.catchTag("HttpClientError", Effect.die)
          )
      ...
      const close = Effect.fn("HttpDiscovery.close")(
        function*(targetId: TargetId) {
          yield* client.get(`${baseUrl}/json/close/${targetId}`).pipe(
            Effect.catchTag("HttpClientError", Effect.die)
          )
        }
      )
  ...
  static readonly layer = (config: typeof CdpConfig.Type): Layer.Layer<HttpDiscovery> =>
