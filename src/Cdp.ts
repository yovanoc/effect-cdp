...
import { Cause, Context, Duration, Effect, Exit, Layer, Ref, Schema, Stream } from "effect"
...
export type CdpError = CdpDecodeError | CdpDisconnected | CdpProtocolError | CdpTimeout
...
const decodeError = (raw: unknown) => (error: Schema.SchemaError): CdpDecodeError =>
  new CdpDecodeError({
    raw: renderRaw(raw),
...
export class Cdp extends Context.Service<Cdp, CdpService>()("Cdp", {
  make: Effect.fnUntraced(function*() {
    ...
    const sendCommand = Effect.fnUntraced(function*<Params, Result>(
      ...
      const encodedParams = yield* effectFromExit(Schema.encodeExit(cmd.params as Schema.Codec<Params, unknown, never, never>)(params)).pipe(
        Effect.mapError(decodeError(params))
      )
      ...
      const trackedSend = pendingCounter === undefined
        ? sendEffect
      ...
      return yield* effectFromExit(Schema.decodeUnknownExit(cmd.result as Schema.Codec<Result, unknown, never, never>)(rawResult)).pipe(
        Effect.mapError(decodeError(rawResult))
      )
    })
    ...
    const root: CdpSession = {
      ...
      events: allEvents.pipe(Stream.filter((event) => event.sessionId === undefined))
    }

    const session = (sessionId: SessionId): CdpSession => ({
      send: (cmd, params, opts) => sendCommand(cmd, params, { ...opts, sessionId }),
      events: allEvents.pipe(Stream.filter((event) => event.sessionId === sessionId))
    })
  ...
  static readonly layerNode = (config: typeof CdpConfig.Type): Layer.Layer<Cdp, never, never> =>
    Layer.effect(Cdp, Cdp.make()).pipe(
      Layer.provide(CdpConnection.layer(config, NodeSocket.layerWebSocketConstructor)),
      Layer.orDie
    )

  static readonly layerBun = (config: typeof CdpConfig.Type): Layer.Layer<Cdp, never, never> =>
    Layer.effect(Cdp, Cdp.make()).pipe(
      Layer.provide(CdpConnection.layer(config, BunSocket.layerWebSocketConstructor)),
      Layer.orDie
    )
}
