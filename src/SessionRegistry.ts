...
export class SessionRegistry extends Context.Service<SessionRegistry, {
  readonly attach: (
    sessionId: SessionId,
    targetId: TargetId,
  ...
  readonly get: (sessionId: SessionId) => Effect.Effect<Option.Option<SessionRegistryEntry>>
  readonly getAll: () => Effect.Effect<ReadonlyArray<SessionId>>
}>()("SessionRegistry") {
  static readonly make: Effect.Effect<SessionRegistry["Service"]> = Effect.gen(function*() {
    const sessions = yield* Ref.make(HashMap.empty<SessionId, SessionRegistryEntry>())
    ...
    const attach = Effect.fnUntraced(function*(
      ...
      yield* Ref.update(
        ...
        HashMap.set(sessionId, {
          targetId,
          parentSessionId: parentSessionId === undefined ? Option.none() : Option.some(parentSessionId)
        })
      )
    })
  ...
  static readonly layer: Layer.Layer<SessionRegistry> = Layer.effect(SessionRegistry, SessionRegistry.make)
}
